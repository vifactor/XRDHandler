...
from RSMPeak import RSMPeak, xu
...
class PeakFitDialog(wx.Dialog):
    def __init__(self, *args, **kwds):
        ...
        # TODO get gridder from MplPanel
        self.gridder = xu.Gridder2D(150,150)
    ...
    def UpdateQxScan(self):
        self.axes_qx.cla()
        #experimental scan
        qx,qxint = xu.analysis.line_cuts.get_qx_scan(self.gridder.xaxis, self.gridder.yaxis,
             self.gridder.data, self.peak.pos_y)
        self.axes_qx.semilogy(qx, qxint, "k-")
        #fit scan
        #x = np.linspace(xmin, xmax, 100)
        qxint_fit = xu.math.functions.Gauss2d(qx, self.peak.pos_y,
                self.peak.pos_x, self.peak.pos_y, self.peak.sigma_x, self.peak.sigma_y, self.peak.scale, self.peak.background, self.peak.angle)
        self.axes_qx.semilogy(qx, qxint_fit, "rx-")

    def UpdateQzScan(self):
        self.axes_qz.cla()
        #experimental scan
        qz,qzint = xu.analysis.line_cuts.get_qz_scan(self.gridder.xaxis, self.gridder.yaxis,
             self.gridder.data, self.peak.pos_x)
        self.axes_qz.semilogy(qz, qzint, "k-")
        #fit scan
        #z = np.linspace(ymin, ymax, 100)
        qzint_fit = xu.math.functions.Gauss2d(self.peak.pos_x, qz,
                self.peak.pos_x, self.peak.pos_y, self.peak.sigma_x, self.peak.sigma_y, self.peak.scale, self.peak.background, self.peak.angle)
        self.axes_qz.semilogy(qz, qzint_fit, "rx-")
